...
UnitRenderable::UnitRenderable(Shader& _shader, unsigned short num_vertices)
...
IndexedRenderable::IndexedRenderable(Shader& shader)
	: shader(shader)
{
	gen_dynamic_vao(vao, vb, ib, 0, shader.stride, 0, varr.data(), iarr.data(), shader.attributes);
}
